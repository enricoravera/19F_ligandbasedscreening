...
from f_fit_260722_17 import fit_exponential, fit_exponential_Jmod, extract_R2_uncertainty, plot_exponential_fit, filter_outliers_fit_R2
...
def collect_paths(
    ...
    if workflow == "titration":
        ...
        # Resolve fit_KD and (if fitting) n_sites once for the whole run,
        # rather than re-asking for every compound — both are workflow-wide
        # choices, and re-asking per compound (as a previous version of
        # this script did) is redundant and easy to answer inconsistently
        # across compounds by mistake.
        fit_KD_cfg = wf_cfg.get("fit_KD", None)
        if fit_KD_cfg is None:
            fit_KD_input = input("  Fit KD value? (y/n) [n]: ").strip().lower()
            DoFit = fit_KD_input == "y"
            wf_cfg["fit_KD"] = DoFit
            _write_fit_KD(config_path, DoFit)
        else:
            DoFit = bool(fit_KD_cfg)

        if DoFit and "n_sites" not in wf_cfg:
            n_sites_input = input(
                "  Number of equivalent binding sites per protein "
                "(n, used in the KD fit) [1]: "
            ).strip()
            try:
                n_sites_val = int(n_sites_input) if n_sites_input else 1
            except ValueError:
                print("    Not a valid integer — using n = 1.")
                n_sites_val = 1
            wf_cfg["n_sites"] = n_sites_val
            _write_toml_scalar(config_path, "[workflow]", "n_sites", f"{n_sites_val}")
            print()

        if DoFit and "fit_R2free" not in wf_cfg:
            # R2,f is directly measurable (at [P]=0) and is fixed by
            # default for exactly that reason — see workflow_titration's
            # docstring for why freeing it is off by default (it reopens
            # a third degree of freedom on top of the already
            # ill-conditioned KD/R2,b fit). Only offer this if the
            # measured [P]=0 point is suspected to be unrepresentative.
            fit_R2free_input = input(
                "  Also let R2,free float in the fit, instead of fixing it "
                "at the measured value? (y/n) [n]: "
            ).strip().lower()
            fit_R2free_val = fit_R2free_input == "y"
            wf_cfg["fit_R2free"] = fit_R2free_val
            _write_toml_scalar(config_path, "[workflow]", "fit_R2free", f"{fit_R2free_val}")
            print()
...
def _format_field_dir(field_T) -> str:
    ...
    ``csar`` runs at two field points (``B_high``/``B_low``) rather than
    one — pass a ``(B_high, B_low)`` pair (or any iterable of floats) to
    get them combined into a single fragment, highest first, e.g.
    ``(16.4, 28.2) → '28p2T_16p4T'``, so the whole run's output lands in
    one directory rather than being split per field point.
...
def _basedir(
    ...
    protein name when one is known, further by protein concentration
    for ``csar``/``fastcsar`` workflows, and further still by the
    magnetic field(s) used for the whole run.
    ...
    ``.../{field}T/`` on top of that when ``run_field`` is provided — a
    single float for fastcsar/titration (one field for the whole run), or
    a ``(B_high, B_low)`` pair for csar (see :func:`_format_field_dir`) —
...
def load_experiment(
    ...
    run_field : float or tuple of float, optional
        Magnetic field(s) for the *whole run* (not just this call's
        ``field_T`` point) — a single float for fastcsar/titration, or
        a ``(B_high, B_low)`` pair for csar. Appends a ``{field}T``
        directory component via :func:`_basedir`, matching
        :func:`fit_experiment` and ``csar_workflows.py``.
...
def fit_experiment(
    ...
    run_field : float or tuple of float, optional
        Magnetic field(s) for the *whole run* — a single float for
        fastcsar/titration, or a ``(B_high, B_low)`` pair for csar.
        Appends a ``{field}T`` directory component via :func:`_basedir`,
        matching :func:`load_experiment` and ``csar_workflows.py``.
...
def run_fitting(
    ...
    # directory (see _basedir/_format_field_dir) so results from runs
    # at different field strengths don't collide — read from the
    # [workflow] keys collect_paths already resolved/persisted earlier
    # (B_high/B_low for csar, B for fastcsar/titration). Reporter has no
    # field-dependent measurement, so run_field stays None for it.
    # csar_workflows.py's run_from_config reads the identical keys, so
    # both scripts land on the same directory.
    run_field: Optional[object] = None
    if workflow == "csar" and "B_high" in wf_cfg and "B_low" in wf_cfg:
        run_field = (float(wf_cfg["B_high"]), float(wf_cfg["B_low"]))
    elif workflow in ("fastcsar", "titration") and "B" in wf_cfg:
